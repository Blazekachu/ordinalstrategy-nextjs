    ...
    <div className="min-h-screen bg-[#0b0c10] text-white relative">
      ...
      {showGate && (
        <div className="fixed inset-0 z-[10000]">
          <canvas ref={gateCanvasRef} className="absolute inset-0 w-full h-full" style={{ display: 'block', backgroundColor: '#000' }} />
          <div className="absolute inset-0 flex items-center justify-center p-4" style={{ zIndex: 10 }}>
            <div className="relative max-w-[720px] w-full bg-black/90 border border-[#f7931a]/45 rounded-2xl shadow-2xl p-5 md:p-7 text-center font-mono backdrop-blur-sm" style={{ pointerEvents: 'auto' }}>
        ...
        <>
          ...
          <div className={`fixed left-0 right-0 bottom-[72px] z-[1001] bg-[#0f1116] backdrop-blur border-t border-white/10 border-b border-white/6 px-4 md:px-[10%] py-0.5 transition-opacity duration-200 ${contentReady ? 'opacity-100' : 'opacity-0'}`}>
            <div className="flex flex-wrap justify-center gap-2 md:gap-3 text-[#f7931a] font-semibold text-xs md:text-sm">
              <span className="magnify-target">🟠 Block: {blockHeight}</span>
              <span className="magnify-target">Txns: {mempoolCount}</span>
              <span className="magnify-target">Fee: {avgFee} sat/vB</span>
            </div>
          </div>
          ...
          <Link href="/foxjump" target="_blank">
            <div className={`magnify-target fixed top-3 right-3 md:right-11 z-[1003] flex items-center gap-1 md:gap-2 cursor-pointer hover:scale-110 transition-all duration-200 ${contentReady ? 'opacity-100' : 'opacity-0'}`}>
